...
成功的 `publish apply` 才使 knowledge 成为唯一可信事实，并自动刷新 AI 派生层与 knowledge SQLite 索引。若响应 warning 表示刷新失败，知识发布仍成立，但应交给 `$llm-wiki-maintain` 修复可重建层。不得直接写 `knowledge/` 或 `llm-wiki/`。
